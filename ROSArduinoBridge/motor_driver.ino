...
void setMotorSpeeds(int leftSpeed, int rightSpeed) {
  ...
  setMotorSpeed(RIGHT, righ tSpeed);
}


#elif defined SMART_DRIVE_DUO
/* Wrap the motor driver initialization */
void initMotorController() {
...
void setMotorSpeed(int i, int spd) {
  if (i == LEFT) {
    if (spd >= 1) {
      ...
      analogWrite(LEFT_MOTOR_SPEED, spd * -1);
    ...
    if (spd >= 1) {
      ...
      analogWrite(RIGHT_MOTOR_SPEED, spd * -1);
...
void setMotorSpeeds(int leftSpeed, int rightSpeed) {
  if (leftSpeed >= 1) {
    ...
    analogWrite(LEFT_MOTOR_SPEED, leftSpeed * -1);
    digitalWrite(LEFT_MOTOR_ENABLE, HIGH);
  }
  ...
  if (rightSpeed >= 1) {
    ...
    analogWrite(RIGHT_MOTOR_SPEED, rightSpeed * -1);
    digitalWrite(RIGHT_MOTOR_ENABLE, HIGH);
  }
}
